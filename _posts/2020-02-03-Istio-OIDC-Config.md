...
Istio is a service mesh that allows you to define and secure services in your Kubernetes mesh. In my lab, I use it as the ingress gateway for my cluster, and I am planning on using it to secure service-to-service communication using mutual-tls.

In this blog post, we will look at the first part of my ideal setup, which is to secure inbound communication via an authenticating reverse proxy, and Keycloak, and how to integrate it into Istio.
...
We will also need to setup OAuth2_Proxy, which performs the OIDC flow for unauthenticated users. We will need to use the Redis session store, as having the ID Token sent to the browser results in cookies that are too long, and splitting the cookie into parts does not work with Istio (see https://pusher.github.io/oauth2_proxy/configuration#configuring-for-use-with-the-nginx-auth_request-directive for an example that spits the cookies with Nginx Ingress support).
...
Istio requires some setup to work properly. On top of the certificate requirement, here are some example config files for setting up basic services in Istio.
...
The Envoy Filter, which is the part that starts the redirection for unauthenticated users. This is similar to the auth-url function of Nginx-Ingress.
